...
Nesse esquema de substituição existe a escolha aleatória sobre qual elemento da cache deve ser substituída.
...
Nesse esquema de substituição quando ocorre um CACHE MISS e é necessário trazer uma nova posição da memória principal, para a memória cache então será removido a posição de memória que está na cache que foi usada há mais tempo.
...
Observe que ao final da execução os elementos que não estão mais na cache são: 0, 1, 2, 3, pois após serem inseridos na cache, os mesmos não mais foram acessados fazendo assim com que eles fossem removidos para que os demais elementos fossem utilizados.
